...
%-----------------------------Write Your Own Code here that assigns all descriptors -------------
%-----------------------------Write Your Own Code here that assigns all descriptors -------------
index_test = zeros(1,90000);

for i = 1:270000
    des1 = TrainMat(i,:);
    d = EuclideanDistance(des1,C);
    [minv,index] = min(d);
    index_train(i) = index;
end

index_train = zeros(1,270000);

for j = 1:90000
    des2 = TestMat(j,:);
    d = EuclideanDistance(des2,C);
    [minv,index] = min(d);
    index_train(j) = index;
end
...
wordid = 25; %397; %37;% set a random value and we intead to find 20 patches from both training and test
...
for ii = 1:nimages
      ...
      if isshow == 1
        ...
        subplot(1,2,2), bar(BoW(:,ii)), xlim([0 400]);
      end 
end 
...
%Methods: 1-L2 (Euclidean distance); 2- Histogram intersection
method = 2; 
